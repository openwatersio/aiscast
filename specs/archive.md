...
The derive job is [lake/derive.py](../lake/derive.py); its [README](../lake/README.md) carries the full schema, source handling table, and hygiene rules. The catalog is R2 Data Catalog on bucket `ais-lake`, and both query paths work against it: DuckDB attaches the catalog directly, and R2 SQL queries it server-side (the API token needs the R2 Data Catalog permission, plus R2 SQL Read for the latter). Messages keep every position report; downsampling is a consumer choice, not the archive's. Day reruns replace the day's rows, so backfills and parser fixes are reruns. Nightly runs happen on the box but read the bucket, not its disk: [derive.service](../server/deploy/rootfs/etc/systemd/system/derive.service) and derive.timer ship in `server/deploy/rootfs`, so every deploy converges them along with the rest of the box.
