...
| BarentsWatch           | NLOD                       | "Data delivered by BarentsWatch". High-traffic use may be charged                | Yes. It is the same Kystverket data re-served as JSON, so it is only worth adding as a fallback path                                               |
