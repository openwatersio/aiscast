...
Base: `https://ais.openwaters.io` (WebSocket: `wss://`).

All responses are JSON. CORS is open (`Access-Control-Allow-Origin: *`) on the HTTP endpoints. Times are UTC. JSON responses are compressed (`zstd` or `gzip`) when the request sends `Accept-Encoding`.
...
| `exp`, `iat` | unix seconds. `exp` is optional and absent on personal tokens, which never expire. aiscast answers misuse with revocation                                                                                                                                                                                                                                   |
...
| `conns`      | optional cap on concurrent WebSockets for this `sub` (on top of 32 per network address for everyone)                                                                                                                                                                                                                                                        |
| `rate`       | optional cap on messages per second per connection. Excess events are thinned (skipped), and the connection stays up                                                                                                                                                                                                                                        |
| `area`       | optional cap on the total subscribed bounding-box area in square degrees (a 20°×20° box is 400). It does not apply to MMSI-filtered subscriptions. A negative value (`-1`) means no bounding-box subscriptions at all: the token can only follow vessels by MMSI (`/v0` with `FiltersShipMMSI`, `/v1` with `mmsi` and no `bbox`), and `/v1/nmea` is refused |
...
No expiry. Two concurrent connections, 50 messages/s per connection, and 400 square degrees of subscribed area. Publishing is allowed: aiscast credits receptions to `v1:ed25519:<pubkey>` / `http:ed25519:<pubkey>`, an identity label with no trust attached, and the Signal K plugin contributes this way. `bind_ip: true` adds the requester's address as a `cidr` claim, so a UDP station at that address counts as this token's (the token then only works from that address).

10 requests per minute per address: one token is all a client needs. `501` if aiscast has no personal issuer configured. The token is a bearer token naming your key, and there is no proof-of-possession handshake yet. Clients that cannot POST (a sandboxed plugin with GET-only network access) can mint the same token in-band on `/v1/stream` with a `register` frame. The operator mints feeder, peer, partner, and admin tokens offline.

**Tiers.** Limits come from the token's claims. The tiers in [limits.md](limits.md) are the defaults. aiscast treats a personal token whose stations (`v1:<sub>`, `http:<sub>`, and any bound UDP address) delivered at least 1,000 events in the last 24 hours as **feeder** on each connection it makes: 5 connections, 200 messages/s, no area cap, and access to `/v1/nmea`. There is nothing to request. The tier lapses when the station stops and returns when it resumes. Without a token, `/v1/stream` and `/v1/nmea`'s HTTP siblings use the **anonymous** tier keyed by address: 2 connections, 20 messages/s, 100 square degrees, subscribe only.
...
- Each box is two `[lat, lon]` corners in any order, and aiscast ORs several boxes together. Latitude −90…90, longitude −180…180.
- `FiltersShipMMSI`: up to 50 nine-digit strings (more if the token's `mmsis` claim allows). When it is present, the list bounds the traffic, so the token's `area` cap does not apply to the boxes (a world box plus an MMSI list is fine on a personal token). The tier's MMSI cap still applies (`Too Many MMSI Filters For This Key`). `FilterMessageTypes`: any of the 24 aisstream type names, with duplicates rejected.
...
- `subscribe`: `bbox` is a list of boxes, each `[minLat, minLon, maxLat, maxLon]`, and `mmsi` is a list of vessels to follow wherever they are. Give either or both. An event matches if it is inside a box OR from a listed MMSI, and MMSI matches include positionless messages such as static data. Sending neither means everything, and only a token without an `area` cap may do that. A token with a negative `area` must send `mmsi` and no `bbox`. The `area` cap applies to the boxes, and the tier's MMSI cap applies to the list (anonymous 10, personal 50, feeder 200, and `{"type":"error","error":"too many mmsi for this key"}` otherwise). Re-sending replaces the subscription. aiscast sends no events until the first subscribe. Without a token the socket has the anonymous tier: 2 concurrent connections per address, 20 messages/s, 100 square degrees, subscribe only. If the token carries `bbox`, every requested box must fit inside one of them, and the total area must fit `area`. Otherwise the reply is `{"type":"error","error":"bbox not allowed for this key"}` and the subscription stays unchanged.
- `snapshot`: with `true`, the subscription starts by replaying the last known messages for every vessel it already matches (positions held up to 30 minutes, and a static message when a name or ship type is known). Replayed frames carry their original `time`. Live events can arrive interleaved with the replay, and a vessel can appear in both, but nothing falls in the gap between them. Most replayed frames are the messages as received. When the original is no longer held (the vessel survived a server restart, or its static data arrived as type 24 halves), the frame is a reconstruction from the vessel cache, marked `synthesized: true` with no `nmea` and no `id`. The replay does not count against the connection's messages/s rate.
- `unsubscribe`: stop receiving events. The socket stays open for publishing.
- `publish`: needs a token (`personal`, `feeder`, `peer`, or `admin`). aiscast ingests the sentences exactly like UDP input (TAG blocks honoured, multipart reassembled per sender, deduplicated) with `source: v1:<sub>`, and answers every frame in order with `{"type":"ack","n":<sentences accepted>}`. `replay: true` marks an offline backlog: sentences whose TAG `c:` time is more than 60 s old are then archived and counted, not emitted live and not folded into the vessel cache. At most 1000 sentences per frame and 6000 per minute per key. aiscast drops the rest and leaves them out of `n`. Without a token: `{"type":"error","error":"publish requires a token"}`.
- `register`: the in-band twin of `POST /v1/keys`, for clients that cannot POST. On an anonymous socket only (`"already registered"` otherwise), it mints a personal token with the same semantics, limits, and `bind_ip` behaviour as the HTTP endpoint, answers `{"type":"key","token":"ak1....","claims":{...}}`, and upgrades this connection to the token's tier in place — no reconnect — confirmed by a fresh `welcome` with the new limits. The connection keeps a live subscription. Errors are non-fatal error frames: `"personal tokens not enabled"`, `"rate limited"` (shares the `/v1/keys` per-address limit), `"pubkey must be a base64url ed25519 public key"`, or a concurrent-connection message when the token's slots are already taken (the socket then stays anonymous).

aiscast → client. The first frame on every accepted socket is a `welcome` advertising the limits in effect for this connection, so a client can size its bounding boxes, pace itself, and set its reconnect delays without discovering the limits by error (a refused socket — bad token, connection caps — gets a single `error` frame instead, never a welcome):
...
- `source`: `kystverket`, `barentswatch`, `digitraffic`, `aishub`, `aisstream`, `http:<station>`, `udp:<hash>`, `mmsi:<n>` (a UDP sender identified by its own AIVDO), `v1:<sub>`. `station` refines it (Kystverket base station id; BarentsWatch stream `terra`/`satellite`/`offshore`). `channel` is `A`/`B`, or empty for events rebuilt from a non-NMEA source.
...
- `synthesized`: `true` when the message was not heard over VHF: rebuilt from a non-NMEA source (BarentsWatch or Digitraffic JSON, AISHub rows, aisstream envelopes), an own-ship report a vessel built from its GPS (`signalk-aiscast` with TAG `s:self`, station `v1:<sub>/self`), or a snapshot reconstruction from the vessel cache. Never fed to AISHub.

Other frames: `{"type":"error","error":"invalid token"}` followed by close 1008 for a bad token. `{"type":"error","error":"bad frame"}` or `"unknown type"` for malformed input. `"concurrent connections per key exceeded"` then close. Inbound frames are limited to 256 KB. aiscast closes slow clients with 1008 "client too slow".
...
`kind` is `vessel`, `aton`, `base`, or `sar`. `type` is the AIS ship/cargo code (or AtoN type). `cog`, `sog`, `heading`, `nav_status`, `name`, and `type` are present only when known: aiscast omits the AIS "not available" sentinels rather than reporting them as 360/102.3/511/15. `seen`, `source`, `station`, and `msg_type` describe the last message that updated the vessel.
...
[
  ...
  },
  {
    "station": "udp:84a377dcf41b",
    "source": "udp:84a377dcf41b",
    "events": { "last_24h": 1064, "last_7d": 6210 },
    "duplicates": 2,
    "vessels": 53,
    "positions": 60,
    "...": "..."
...
`GET /v1/stations/{id}` returns `{"station": {...same row...}, "vessels": <GeoJSON FeatureCollection of the vessels this station last updated>}`. An unknown id gets `404`. The viewer shows it at `?station=<id>`.
...
```json
{
  ...
  "vessels": {
    "total": 4812,
    "with_position": 4790,
    "by_kind": { "vessel": 4701, "aton": 88, "base": 19, "sar": 4 }
  },
  "events": {
    "per_second": 212.4,
    "last_24h": 18230411,
    "last_7d": 121004312,
    "duplicates": { "last_24h": 2210560, "last_7d": 15320011 }
  },
  "clients": {
    "streams": 9,
    "streams_opened": { "last_24h": 410, "last_7d": 2822 },
    "requests": { "last_24h": 28310, "last_7d": 190412 }
  },
  "sources": {
    "kystverket": {
      "events": { "last_24h": 9120033, "last_7d": 61233190 },
      "last_age_s": 0,
      "vessels": 2411,
      "vessels_exclusive": 180
    },
    "v1": {
      "events": { "last_24h": 40211, "last_7d": 281002 },
      "last_age_s": 3,
      "vessels": 61,
      "vessels_exclusive": 2
    },
...
WebSocket. Token with role `feeder`, `peer`, `partner`, or `admin`, or a personal token that has earned the feeder tier by contributing (`?key=` or `Authorization: Bearer`). aiscast refuses anonymous connections. The feed carries a low-trust (UDP) event only after a trusted source has corroborated it. Optional `?bbox=minLat,minLon,maxLat,maxLon` (repeatable) filters by the vessel's last known position. The token's `bbox`, `area`, `conns`, and `rate` claims apply.
...
aiscast delivers synthesized messages (Digitraffic, AISHub, aisstream) as re-encoded `!AIVDM` with `t:` naming their terms. Filter on `t:` if you only want receiver data.

## `POST /v1/receive`

AIS-catcher's HTTP output: `AIS-catcher -H https://ais.openwaters.io/v1/receive USERPWD x:<token> GZIP on INTERVAL 15`. Body is either AIS-catcher's `jsonaiscatcher` envelope (`{"msgs":[{"nmea":["!AIVDM,..."],"rxtime":"20260820111900","channel":"A"}, ...]}`) or plain newline-separated NMEA. `Content-Encoding: gzip` is accepted, with a 1 MB limit before and after decompression. Needs a token that may publish (`personal`, `feeder`, `peer`, `admin`). The token's `sub` becomes the station (`source: http:<sub>`). `rxtime` is used as the message time when within 30 s of arrival. 600 posts per minute per station. Responses: `200` empty, `401` with a reason, `413`, `429`.

## UDP `ais.openwaters.io:10110`

Datagrams of newline-separated NMEA (`!AIVDM`/`!AIVDO`, `!BSVDM`…, TAG blocks allowed, lines ≤ 4 KB). No authentication, 500 sentences per second per source address. aiscast identifies the sender as `udp:<keyed hash of address>`. If the sender sends `!AIVDO`, aiscast identifies it as `mmsi:<own MMSI>` from then on. If a personal token was minted with `bind_ip` from the same address, the station counts toward that token's feeder tier.

UDP is the lowest-trust path. aiscast shows its events on the map and in `/v1/stream` with their `source`, but they never raise a vessel's trust. aiscast drops a position that implies more than 120 knots from the vessel's last known position: it archives that position and does not emit it. aiscast forwards UDP events to AISHub and `/v1/nmea` only while a trusted source (an open feed, an authenticated station) has heard the same vessel within the last hour. Everything received is archived with its source, so aiscast can purge a bad source.

## Tolerances and dedupe

Accepted on every input: NMEA 4.10 TAG blocks (`s:` station, `c:` time in s/ms/µs, `g:` grouping), any talker (`AI`, `BS`, `AB`…), channels `A`/`B`/`1`/`2`, fixed-length messages up to 5 bits over length, USCG trailing fields after the checksum (`…*5B,s36310,d-081`). aiscast reassembles multipart messages per sender. It drops a duplicate when the same payload on the same channel arrives within 10 s of canonical time, and whichever station heard it first wins. Out-of-skew source timestamps fall back to receive time.

## Limits

Defaults by tier, which a minted token's claims override. See [limits.md](limits.md) for the reasoning.
...
- A 1,024-event send queue, after which aiscast disconnects the client.
