...
| Archive | Every reception, source-native, hourly gzip per source to R2 with the license tag in the object path. Beside it, the [normalized archive](normalized-archive.md) records every accepted event, copy, and weather broadcast in one merged stream | Receptions are the lossless record, and R2 has no egress fee. The normalized stream is what downstream reads, so nothing downstream re-decodes |
