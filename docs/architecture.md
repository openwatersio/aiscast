...
| Client protocol | aisstream.io-compatible WebSocket at `/v0/stream`, frozen. `/v1/stream` is a bidirectional event stream (publish and subscribe on one socket) carrying station, raw body, and receive time; the same URL serves SSE to a plain GET and MQTT to a socket that asks for that subprotocol | Existing aisstream clients work unchanged, and `/v1` is also the peer protocol, so feeders, nodes, boats, and chart clients all speak one thing |
| Snapshot | `GET /v1/vessels?bbox=` returns current state | Without it a client waits minutes for a view to fill |
| AI assistants | `/mcp`: MCP over Streamable HTTP, stateless, plain JSON, no sign-in. Five read-only tools over the vessel cache and station stats, the same claims, tiers, and rate limit as `/v1/vessels` | Assistants ask one question at a time and cannot hold a WebSocket. The cache already answers those questions, so the tools are thin and no second service exists |
...
| Own-ship reports | A station's `!AIVDO` names the station and marks a self-report at ingest. `/v1/nmea` renders it as `!AIVDM`; the archive and the `nmea` array on a `/v1` event keep it as it arrived | VDO means "the receiver's own vessel", so on the raw feed it makes a consumer treat the contributor's boat as itself: OpenCPN moves own ship to that position and drops the vessel from the target list and from CPA. `/v1` consumers have `synthesized` and the station beside the sentence, and a peer needs the original to derive them again |
| Feeder ingest | Receive-only MQTT 3.1.1 on `/v1/stream` when the socket negotiates the `mqtt` subprotocol, token in CONNECT, NMEA per PUBLISH, SUBSCRIBE refused. HTTP POST of AIS-catcher's `jsonaiscatcher` envelope with gzip and Basic auth, content-sniffing `LIST`/`NMEA` bodies. `/v1/stream` publish for peers. UDP NMEA as a legacy adapter | Matches what existing receivers already emit. AIS-catcher's MQTT output publishes each message as decoded, where its HTTP output batches with a one-second floor, so MQTT is the realtime path with station credit. The packet subset is hand-rolled: an embedded broker would need fencing off subscriptions, which would bypass every tier limit. HTTP and WebSocket work through any NAT, and UDP goes direct |
...
| Metrics and health | Prometheus `/metrics` (per-source rate, last-message age, and delay, dedupe ratio, streams by protocol and tier, request latency, queue drops, invalid timestamps per station), box-local. Grafana Alloy on the box ships it with Caddy and host metrics to Grafana Cloud, where the alert rules and capacity dashboard from `server/deploy/grafana/` run. `/health` fails when a loopback subscriber has received nothing for 2 minutes | The aisstream failure mode was a healthy-looking empty service. Metrics and alerts held off the box survive the box |
...
Golden tests hold the line: per-type fixtures built from captured aisstream output and the `ais-message-models` schema, subscription validation cases, static-message routing through the position cache, and slow-client behavior. Anything new goes under `/v1`, where additive changes are fine. Source kinds on `/v1` are the part of `source` before the first colon: an upstream by name, `station` for every authenticated contributor whatever transport delivered it, `udp` and `mmsi` for unauthenticated senders. Consumers key on the kind; the transport is never encoded in it. `/mcp` is the one path outside it: MCP carries its own protocol version in every request, and every client and registry expects that path.
