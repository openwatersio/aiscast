...
- **AIS-catcher** (preferred: authenticated, realtime, works behind any NAT): get a token at [openwaters.io/ais/token](https://openwaters.io/ais/token) (one click, stays in your browser), then `AIS-catcher ... -Q wssmqtt://x:<token>@ais.openwaters.io:443/v1/stream MSGFORMAT NMEA`. Each message is sent as it is decoded, over MQTT on the same WebSocket address the JSON stream uses. Your data appears as `source: station:<station id>`. Ask for a named station with higher limits. AIS-catcher's HTTP output works too: `-H https://ais.openwaters.io/v1/receive USERPWD x:<token> GZIP on INTERVAL 5` posts a batch every five seconds under the same station.
...
| Volunteer receivers    | The [contributor agreement](docs/contributor-agreement.md) dedicates receptions to the public domain under CC0 and publishes the aggregate under ODbL | Credit "Open Waters AIS (https://openwaters.io/ais/)". Individual receptions are CC0. The aggregate carries ODbL.                                                         |
