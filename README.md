...
| Volunteer receivers    | beta: contributors give this server permission to re-serve their data. A written feeder agreement is the next stage: an open license on the aggregate, no transfer to any acquirer, opt-in, and station locations never published precisely. It is reviewed before volunteer data scales | Credit "aiscast volunteer receivers" for now. Expect an open-data license (ODbL is the proposal) once the agreement exists.                                                                       |
...
aiscast's own code is [MIT](LICENSE). aiscast never publishes precise volunteer station locations. It identifies UDP stations by a keyed hash instead of an address. The archive keeps raw receptions per source, so the project can purge any source. [docs/policy.md](docs/policy.md) states the full position: what the feeder agreement must say, the privacy rules, and how the project intends to fund itself without relicensing data.
