...
```
  ...
  volunteer stations ─UDP/HTTP─▶ │  aiscast (Go, one process)         │ ──WS──▶ Cloudflare proxy ──▶ clients
...
| Host               | Hetzner Cloud EU (20 TB/mo included, €1/TB over): `ais-server-1` is a CX43 (8 shared vCPU, 16 GB, €18.49/mo) in Helsinki because measured fan-out is ~12% of a core per 1,000 clients; CCX23 (dedicated, €101.49/mo) is the resize if CPU ever shows. "Hetzner-class" means any cheap-bandwidth VPS with a dedicated IP (OVH, Scaleway, Oracle are the alternatives; hyperscalers are 5–10× on egress) | origin egress decides it: Fly bills $0.02/GB on every byte sent to Cloudflare, Hetzner includes 20 TB ([host table](research/infrastructure.md#design-b-single-binary-on-a-vps)) |
...
| Single-node fan-out capacity      | 1,000 clients × 68 msg/s = 68k sends/s, 37.7 MB/s, zero queue drops, 57 MB RSS, ~12% of one laptop core | measured 2026-08-20 with `cmd/loadtest` against aiscast on an M-series laptop, uncompressed frames; 250k sends/s with batching still extrapolated |
...
Long term aiscast is one peer in a network where vessels, shore receivers, nodes, and chart clients exchange AIS as signed events over the internet, alongside VHF. Nothing in that future is built now, but three cheap early decisions keep it reachable:

1. `/v1/stream` is direction-agnostic: one WebSocket, publish and subscribe frames, bbox/cell subscriptions. Node-to-node peering is then just a client that also publishes.
2. The internal record is the event above, with a content hash id, an origin key, a geo cell, contributing receptions, and room for a signature. Dedupe by id; provenance by signature chain. aiscast relays; it signs only its own observations and attestations.
...
1. Repo `server/` (name TBD), Go module: `source/` (Kystverket TCP), `ingest/` (HTTP POST, UDP), `pipeline/` (reception → reassembly per source → dedupe → decode → event → state), `stream/` (WS server, bbox filter, `/v0` rendering, bidirectional `/v1/stream`), `archive/` (hourly source-native gz → R2 via S3 API).
...
Code: [`server/`](server/README.md). Simplifications to revisit in 0B: UDP station = sender IP (not per-station ports/keys), R2 upload shells out to wrangler, the open hour is not uploaded on shutdown, no `/metrics` or rate limits yet.

### Stage 0A½: viewer

A static map of live traffic, the first client of aiscast other than aisstream-compatible ones. Lives in [`viewer/`](viewer/) at the repo root, not in aiscast: aiscast stays a pure API, the page is a client like the chart plugin, it deploys to Cloudflare Pages for free, and it becomes the public "what's on the feed" and coverage page later.

- One `index.html`, vanilla JS + MapLibre GL, no build step. `?server=` selects the server (default `localhost:8080`). OpenFreeMap basemap until the Open Waters tiles are used. `hash: true` for shareable positions.
...
- aiscast side, pulled forward from 0B: vessel cache carries COG/SOG/heading/type/nav status/last seen/source/station and evicts idle vessels; `GET /v1/vessels?bbox=` returns GeoJSON with `Access-Control-Allow-Origin: *`.
...
- [x] Box: `ais-server-1` on Hetzner (hel1, CX43), systemd, firewall, server live at `http://2.29.0.215:8080` and `udp://2.29.0.215:10110` with Kystverket + Digitraffic ([deploy/README.md](server/deploy/README.md)). [x] `https://ais.openwaters.io` live: Cloudflare proxied → Caddy (Let's Encrypt) → server on localhost; 8080 closed. [x] Archive hours upload to R2 from the box over the S3 API (hand-rolled SigV4 PUT). [x] `AISSTREAM_API_KEY` on the box (aisstream connected but delivered nothing during setup; it is best effort and outside the health gate). [x] `ais.openwaters.io` is DNS-only (no proxy) for the beta, so the same name serves UDP ingest; `TRUST_CF_HEADERS` gates Cloudflare header trust if proxying is ever turned on. [x] Access tokens replace static keys (below).
...
Kystverket allows one TCP connection per source IP (a second connection makes both reconnect every few seconds): one server per public IP, and the second node in Stage 4 must not also pull Kystverket from the same IP.
...
Identity and authorization are separate. Identity is a device-generated Ed25519 keypair (the Signal K and chart plugins; Stage 3 signs events with it). Authorization is an Ed25519-signed claims token (`ak1.<claims>.<sig>`): `sub`, `role` (`personal`, `feeder`, `peer`, `partner`, `admin`), `exp`, optional `bbox`, `cidr`, `conns`. The issuer private key lives offline (`cmd/aiscast-key`, seed in the untracked `.env`); aiscast verifies with public keys (`ISSUER_PUBKEYS`) and holds only a personal-tier issuer key so `POST /v1/keys` can hand a 30-day personal token to any device key without a bundled secret. The token is accepted wherever a key was: aisstream `APIKey`, Bearer, Basic password, `?key=`. `/v1/stream` subscribe and `/v1/vessels` stay open; `/v0/stream`, publish, and `/v1/receive` need a token. Not done: proof of possession at connect (personal tokens are bearer tokens naming the device key), holder attenuation (macaroon/Biscuit-style caveats) — add when a partner needs to issue sub-keys.

### Stage 0D: openwaters.io pages

The public face of the service is [openwaters.io](https://openwaters.io) (repo `openwatersio/openwaters.io`, Astro + Tailwind, `website/src/pages/`, nav in `components/layout/Header.astro`, footer in `Footer.astro`, `/api` in `pages/api.astro`). On the site the product is called **AIS** (nav, page title, "AIS API"); `aiscast` appears only where the repo, server, or plugin is named. This repo stays the source of truth for everything the site shows (viewer, [docs/API.md](docs/API.md), plugin); the site only renders.

Website repo:

- **Nav**: `AIS` → `/ais` in `navItems` (after Charts) and in the footer Explore list; an AIS card on the homepage next to Charts/Tides/Bathymetry.
- **`/ais`** (`pages/ais/index.astro`, hero pattern of `/charts`): hero with the README's framing (live vessel traffic by bounding box from open government feeds and volunteer receivers; beta, no SLA, coverage uneven), `<iframe src="https://openwatersio.github.io/aiscast/#8/59.5/10.6">` full-width under it (Pages sends no frame-blocking headers; the hash sets the framing), then sections with a TOC like `/charts/seamap`, each a condensed copy of the README section it mirrors: **Use the data** (`/v1/vessels` and a `/v1/stream` subscribe frame as copy-paste examples, `/v0/stream` for aisstream.io clients with the hostname swap and a token as `APIKey`, personal tokens via `POST /v1/keys`, link to `/api/ais`), **Signal K** (`signalk-aiscast` on npm: shares what the receiver hears, shows traffic when it has none; `signalk-aisstream` works with the hostname swap), **Contribute data** (UDP `ais.openwaters.io:10110` with no token, AIS-catcher HTTP with a feeder token on request, `/v1/stations` to see your station arrive; beta wording until the feeder agreement exists), **Coverage** (the README table), **License** (the per-source table with the attribution strings consumers must carry; the aggregate is not relicensed; "not for navigation" callout: positions are drawn where reported, aggregate sources are minutes old, no collision avoidance), **Code** (repo, CONTRIBUTING). No live counters (`/v1/vessels` and `/v1/stations` have CORS open, so a vessel or station count is one fetch away when wanted).
- **`/api/ais`**: [docs/API.md](docs/API.md) rendered as-is. An Astro content-layer loader fetches `raw.githubusercontent.com/openwatersio/aiscast/main/docs/API.md` at build time and `renderMarkdown`s it; the page wraps `<Content />` in `prose` (`@tailwindcss/typography` is already installed). Fetching from GitHub, not from `ais.openwaters.io`, so a server outage cannot fail a site build. `/api` (the tides page) gets an "AIS API" section linking to it. Not an OpenAPI spec: two of the endpoints are WebSockets and most of the reference is frame and token prose that OpenAPI cannot carry; revisit if a generated client or Swagger UI is ever wanted.
- **`/api` host**: `ApiEndpoint` hard-codes `API_HOST` (`api.openwaters.io`); untouched, since AIS renders from markdown with `https://ais.openwaters.io` in the text.
...
- [x] Self-serve tokens: `viewer/token.html` makes a device key in the browser and gets a personal token from `POST /v1/keys` (personal tokens may publish as themselves); named `feeder` tokens by hand.
...
- `GET /v1/vessels/{mmsi}` last known; `GET /v1/vessels/{mmsi}/track?from&to`; `GET /v1/history?bbox&from&to`; station stats; coverage heatmap tiles. Served by a Worker over R2 SQL / Iceberg with caching; hot recent window from aiscast's memory.
- Denmark backfill only after written confirmation of reuse rights from DMA (`sifa@brs2.dk`).

### Stage 3: Signal K plugin

`signalk-aiscast`, the vessel agent, in [`signalk-plugin/`](signalk-plugin/) of this repo. One plugin, one socket to aiscast, both directions: it **broadcasts** what the boat's receiver hears and **consumes** aiscast's traffic when it has nothing of its own to hear. Survey of every existing AIS plugin and the server API facts this relies on: [research/signalk-plugins.md](research/signalk-plugins.md). Nothing in the ecosystem does both directions well; the failure modes they share (no reconnect, leaked listeners, silent subscriptions, retry storms, own ship echoed back as a target) are the checklist below.

#### Identity and self-authentication

No registration step. The plugin's identity is an Ed25519 keypair generated on first `start()` and stored in `<getDataDirPath()>/identity.json`; it never leaves the boat. On start (and three days before expiry) the plugin posts the public key to `POST /v1/keys` and receives a 30-day `personal` token whose `sub` is `ed25519:<pubkey>`; that token rides on the `/v1/stream` socket as an `Authorization: Bearer` header. Personal tokens may subscribe and publish, so the boat's receptions are credited to its key (`source: v1:ed25519:<pubkey>`) with no operator involvement. The token is a bearer naming the key, not proof of possession; signed frames and a server `hello` come when federation needs them, and the keypair is already in place for that. An operator-issued feeder or peer token pasted into the config replaces the self-minted one (longer life, higher limits). TLS authenticates the server.

- **Trust**: a self-minted key is an identity label, never a trust upgrade: the same footing as a UDP sender keyed by its AIVDO MMSI. Abuse controls are per-IP connect limits and per-key `conns`; promotion to `corroborated`/`attested` is Stage 1/4 attestation work.
- **Timestamps**: the plugin prepends `\c:<unix ms>*hh\` (TAG block, checksummed) to every sentence that lacks one, at the moment `nmea0183` fired. On the `/v1/stream` publish path a sentence in a `replay: true` frame whose `c:` is more than 60 s old is archived and counted (`aiscast_replayed_total`) but not emitted live and not folded into the vessel cache; that is how an offline backlog replays without stale positions jumping onto the live map.
- **Acks**: every publish frame is answered with `{"type":"ack","n":<sentences>}`, in order; the plugin's disk queue advances only on ack, and replayed frames carry `replay: true` so the 60 s rule never touches live sending on a boat with a wrong clock. `{"type":"unsubscribe"}` drops the subscription without closing the socket, so a boat that starts hearing VHF again keeps publishing on the same connection.
...
- **Loop guards**: drop inbound events whose `station` is our own key (aiscast echoes what we publish); keep payloads received from aiscast in a 5 min set and never publish them back (`signalk-n2kais-to-nmea0183` would otherwise turn our injected deltas into `!AIVDM` on `nmea0183out`, and aiscast would credit the boat with receptions it never made).
...
- `/v1/stream`: `personal` tokens may publish (6000 sentences/min per key, 1000 per frame); `ack` after every publish frame; `unsubscribe` frame; `replay: true` frames are `Buffered`, so a `c:` older than 60 s is archived only.
- `!AIVDO` decodes through the NMEA path (go-nmea `VDMVDO` covers both talkers); own-ship receptions are attributed like any other. `/v1/stream` bbox-routes positionless static messages through the vessel cache's last position, so names reach the plugin.

#### Checklist

Server: [x] personal tokens publish [x] `ack` [x] `unsubscribe` [x] replay rule (`c:` > 60 s → archive only) [x] tests.

Plugin: [x] identity + token + status [x] collect + TAG block + live publish + disk queue + ack [x] backoff, watchdog, token refresh [x] consume: modes, bbox follow, parser injection, drop rules [x] loop guards [x] tests (vitest against a fake aiscast: key persists and token is minted; offline → files → drain on reconnect → deleted on ack; `auto` flips on/off with local AIS; injected delta has the expected context/`$source`/timestamp and own MMSI and own echoes are dropped; hub-received payload is never republished) [x] smoke test against the real server (token minted, reception credited to the key, echo dropped) [ ] end-to-end with a real Signal K server, below.

Verify end to end: Signal K with `--sample-nmea0183-data` + local server → receptions attributed to the plugin's key in the viewer and the archive; stop the sample data → plugin flips to consumer mode and Kystverket targets appear under `vessels.urn:mrn:imo:mmsi:*` with `$source` `signalk-aiscast.net`; pull the network for a minute → queue files appear, drain on reconnect, land in the archive and stay off the live map.

Deferred, with the trigger that brings each back: plain-UDP mode (a user on a link where WS overhead matters; so far nobody); per-vessel downsampling for metered links (`ais-forwarder` #12 shows the demand; needs MMSI extraction on the boat); more than one server (federation, Stage 4); privacy dials for own ship beyond on/off (delayed, coarse, encrypted to a group); filtering GPS-jammed far targets (server-side, once station positions exist); `/v1/vessels` snapshot fill on subscribe (if waiting a few minutes for static data is a real complaint); boat-to-boat mesh sync.
...
- **What is sold is the hosted service:** reliable bbox fan-out, history queries, SLA, and permission to use _this service_ commercially. Precedents: Open-Meteo (open weather data, free API for non-commercial use, paid commercial plans), OpenSky Network (community ADS-B, free for non-commercial research, commercial license), OSM hosted by Mapbox/MapTiler.
...
- Own-ship `!AIVDO` from Signal K: shared once the plugin is enabled (the plugin ships disabled, so enabling it is the consent), with its own switch to turn it off.
...
The Open Waters chart plugin speaks bbox-subscribe over WebSocket regardless of who serves it; its core seams (`VIEW_CHANGED` event, `net` provenance flag) are provider-agnostic, so pointing it at this server needs no core work. The bundled aisstream key moves server-side (aisstream throttles per key). The `net.ws` manifest allowlist holds up to 8 hosts, so server hostname and a backup can be pre-listed to avoid re-consent later.

## Open questions

- Hostname: `ais.openwaters.io` for aiscast (a host-only change keeps aisstream clients working; a path under `api.openwaters.io` would not) and `ingest.openwaters.io` DNS-only for UDP, pending confirmation. Name is settled: `aiscast` for the repo, server, and plugin; "AIS" on openwaters.io.
