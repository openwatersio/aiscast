...
- [x] Box: `ais-hub-1` on Hetzner (hel1, CX43), systemd, firewall, hub live at `http://2.29.0.215:8080` and `udp://2.29.0.215:10110` with Kystverket + Digitraffic ([deploy/README.md](hub/deploy/README.md)). [x] `https://ais.openwaters.io` live: Cloudflare proxied → Caddy (Let's Encrypt) → hub on localhost; 8080 closed. [x] Archive hours upload to R2 from the box over the S3 API (hand-rolled SigV4 PUT). [x] `AISSTREAM_API_KEY` on the box (aisstream connected but delivered nothing during setup; it is best effort and outside the health gate). [ ] `ingest.openwaters.io` DNS-only record; confirm SSL mode Full (strict). [ ] `FEEDER_KEYS`.
...
`signalk-aiscast`, the vessel agent, in [`plugin/`](plugin/) of this repo. One plugin, one socket to the hub, both directions: it **broadcasts** what the boat's receiver hears and **consumes** the hub's traffic when it has nothing of its own to hear. Survey of every existing AIS plugin and the server API facts this relies on: [research/signalk-plugins.md](research/signalk-plugins.md). Nothing in the ecosystem does both directions well; the failure modes they share (no reconnect, leaked listeners, silent subscriptions, retry storms, own ship echoed back as a target) are the checklist below.
...
- **Queue**: every 15 s ± 20 % jitter the collected sentences become one batch file `<dataDir>/queue/<ms>.json` (write-ahead), then one publish frame; the file is deleted on `ack`. Offline, files accumulate; on reconnect they drain oldest-first, one frame in flight. Cap 100 MB, oldest dropped and counted. Per-sentence `c:` carries the real receive time, so the hub's 60 s rule sorts live from replay without a flag.
