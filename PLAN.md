...
- [x] Box: `ais-hub-1` on Hetzner (hel1, CX43), systemd, firewall, hub live at `http://2.29.0.215:8080` and `udp://2.29.0.215:10110` with Kystverket + Digitraffic ([deploy/README.md](hub/deploy/README.md)). [ ] Cloudflare DNS `ais.openwaters.io` (proxied) + `ingest.openwaters.io` (DNS only), Caddy/Let's Encrypt at the origin, Full (strict), close 8080. [ ] R2 API token → S3 client upload from the box. [ ] `FEEDER_KEYS`, `AISSTREAM_API_KEY` on the box.
