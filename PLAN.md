...
- **Publish frame** on `/v1/stream`: `{"type":"publish","pubkey":<hex>,"time":<unix ms>,"nmea":[...],"sig":<hex>}` with `sig = Ed25519(time + "\n" + nmea.join("\n"))`. aiscast verifies the signature, rejects `time` more than 5 min from its clock, and answers `{"type":"ack","time":<frame time>,"n":<accepted>}`. Station and source become `key:<pubkey>`; the whole signed frame is archived as one reception (source-native, signature preserved) alongside the per-sentence receptions. Basic-auth publish and `FEEDER_KEYS` stay for AIS-catcher.
- **aiscast key**: `SERVER_KEY` file, generated on first run. On connect aiscast sends `{"type":"hello","pubkey","time","sig"}` with `sig = Ed25519(time)`; the plugin pins the key on first contact (TOFU) and turns the status line red if it changes. TLS already authenticates the hostname; the hello is what makes server identity survive a hostname move and is the seed for Stage 4 relay signatures.
- **Trust**: self-signed keys are accepted by default and labeled trust `self`; keys cost nothing, so the abuse controls are per-IP connect limits and per-key publish limits, not the key itself. Promotion to `corroborated`/`attested` is the Stage 1/4 attestation work; aiscast only needs a place for a trust tier on the station now.
- **Timestamps**: the plugin prepends `\c:<unix ms>*hh\` (TAG block, checksummed) to every sentence that lacks one, at the moment `nmea0183` fired. aiscast already parses `c:`. On the publish path, a sentence whose `c:` is more than 60 s old is archived and counted but not emitted live and not folded into the vessel cache; that is how an offline backlog replays without stale positions jumping onto the live map.

#### Plugin behavior

- **Collect**: `app.on('nmea0183')` and `app.on('nmea0183out')` (the latter is where `signalk-n2kais-to-nmea0183` puts N2K-derived AIS), regex `^(\\[^\\]*\\)?[!$]..VD[MO],`. `VDM` always (when sharing is on), `VDO` only with the separate own-ship consent. Verbatim except for the TAG block; aiscast decodes, dedupes, and archives. Listeners are removed on the same emitter in `stop()`; `start()` calls `stop()` first.
...
- **Status**: one line, refreshed at most every 5 s: `key 3f9a…  ↑ 42 msg/min (queue 0)  ↓ 118 targets  server ok 2 s ago`; `setPluginError` on signature rejection, pinned-key mismatch, or a queue that has been draining for more than an hour.
- **Config** (JSON schema, no webapp): `server` (WebSocket URL, default the public instance), `share.targets` (default on), `share.ownShip` (default off, its own checkbox with the consent text: where it goes, that it is public, how to stop), `receive.mode`, `receive.radiusNm`. Everything else is a constant.
...
- `server/identity.go`: Ed25519 key file, `hello` frame, signed-publish verification, `ack`, station/source `key:<pubkey>`, trust tier on the station, per-key publish limits.
...
- `/v1/stream` already bbox-routes positionless static messages through the vessel cache's last position, so names reach the plugin without further server work.

#### Checklist

Server: [ ] server keypair + `hello` [ ] signed publish + `ack` + per-key limits [ ] frame archived with signature [ ] replay rule (`c:` > 60 s → archive only) [ ] tests: good/bad signature, skewed `time`, replay frame not emitted, echo carries `station == key`.

Plugin: [ ] identity + status + `/identity` route [ ] collect + TAG block + live publish + disk queue + ack [ ] backoff, watchdog, pinned server key [ ] consume: modes, bbox follow, parser injection, drop rules [ ] loop guards [ ] tests: signature verifies with `node:crypto`; offline → files → drain on reconnect → deleted on ack; `auto` flips on/off with local AIS; injected delta has the expected context/`$source`/timestamp and own MMSI and own echoes are dropped; server-received payload is never republished.
