...
- Traffic from aiscast also goes out as NMEA 0183. Injected targets are re-emitted verbatim on `nmea0183out` (TAG block stripped), which the server's TCP 10110 interface and any connection with `nmea0183out` in its output events relays to chartplotters and tablets — the apps that do not speak Signal K. Two limits, both because a plotter sees a bare `!AIVDM` and cannot tell it from a VHF reception the way a Signal K app can:
  ...
  - Only events received in the last 2 minutes. The snapshot replayed on subscribe, AISHub's ~5-minute aggregate, and late satellite passes stay in the Signal K data model, where they are timestamped and attributed, and off the wire. This also keeps the replay burst off a 4800-baud serial line. Names and dimensions follow the same rule, so a fresh target is unnamed on the plotter until its next static retransmission, up to six minutes.
