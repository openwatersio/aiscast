...
- On the box (from [cloud-init.yaml](cloud-init.yaml)): user `hub`, `/opt/hub/hub`, `/var/lib/hub/{archive,vessels.json}`, `/etc/hub.env` (0600; holds `ISSUER_PUBKEYS`, `PERSONAL_ISSUER_KEY`, the R2 and AISHub/aisstream settings; the issuer *seed* for minting tokens lives only in the repo's untracked `.env` as `ISSUER_SEED`/`ISSUER_KID`), `/etc/systemd/system/hub.service` enabled.
