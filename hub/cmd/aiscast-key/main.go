// aiscast-key mints and inspects hub access tokens. The issuer private key never goes near the hub.
//
...
func main() {
	...
	case "new":
		...
		c := claims{Kid: *kid, Sub: *sub, Role: *role, Iat: time.Now().Unix(), Exp: time.Now().Add(*exp).Unix(), CIDR: cidrs, Conns: *conns}
		for _, b := range boxes {
			var x bbox
