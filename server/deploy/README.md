...
- Hetzner Cloud server `ais-server-1`: `cx43` (8 shared vCPU, 16 GB, 160 GB NVMe, 20 TB/mo traffic, €18.49/mo) in `hel1` (Helsinki), Ubuntu 24.04, IPv4 `2.29.0.215`, IPv6 `2a01:4f9:c015:e7ca::/64`, label `project=ais`. Resize to a dedicated-CPU type (`ccx23`, €101.49/mo) only if fan-out CPU shows up in `/metrics`; the load test ran 1,000 clients at ~12% of one core.
- Firewall `ais-server`: in 22/tcp, 80/tcp, 443/tcp, 10110/udp, ICMP.
- SSH key `bkeepers-ed25519` (the 1Password agent key); root login with it. sshd is key-only with `MaxStartups 50:30:200`, `PerSourceMaxStartups 3`, `LoginGraceTime 20` (`/etc/ssh/sshd_config.d/10-hardening.conf`) and fail2ban (`/etc/fail2ban/jail.local`: 3 tries / 10 min → 1 h ban): the box gets continuous root-password brute force, and with the defaults those attempts filled the pre-auth slots and randomly dropped real connections, including CI deploys.
- On the box (from [cloud-init.yaml](cloud-init.yaml)): user `aiscast`, `/opt/aiscast/aiscast`, `/var/lib/aiscast/{archive,vessels.json}`, `/etc/aiscast.env` (0600; holds `ISSUER_PUBKEYS`, `PERSONAL_ISSUER_KEY`, the R2 and AISHub/aisstream settings; the issuer *seed* for minting tokens lives only in the repo's untracked `.env` as `ISSUER_SEED`/`ISSUER_KID`), `/etc/systemd/system/aiscast.service` enabled.
- Public: `https://ais.openwaters.io` (DNS-only A record → Caddy ([Caddyfile](Caddyfile): Let's Encrypt cert, `zstd`/`gzip` response compression, `/metrics` blocked) → aiscast on `127.0.0.1:8080`; Cloudflare proxying is off for the beta and can be re-enabled with `TRUST_CF_HEADERS=1` if the box needs DDoS cover): `/v0/stream`, `/v1/stream`, `/v1/vessels`, `/v1/receive`, `/health`; `/metrics` is only reachable on the box. UDP ingest at `ais.openwaters.io:10110` (same name; it resolves straight to the box). Upstreams: Kystverket, Digitraffic, aisstream.io (best effort, not in the health gate). Archive hours upload to R2 `ais-archive` over the S3 API on rotation and on shutdown.
...
[`.github/workflows/ci.yml`](../../.github/workflows/ci.yml): every push and PR runs gofmt/vet/test and builds the linux/amd64 binary; a push to `main` then copies it and [Caddyfile](Caddyfile) to the box as the `deploy` user, restarts the service, reloads Caddy (graceful: open connections survive, a bad Caddyfile is rejected and the old config stays), and fails the run if `/health` doesn't answer. The `deploy` user has its own ed25519 key (`/home/deploy/.ssh/authorized_keys`), owns `/opt/aiscast` and `/etc/caddy/Caddyfile`, and can run only `sudo systemctl restart aiscast` / `is-active aiscast` / `reload caddy` / `is-active caddy` (`/etc/sudoers.d/deploy`). Repository secrets: `DEPLOY_SSH_KEY` (that private key), `DEPLOY_HOST` (the box IP), `DEPLOY_KNOWN_HOSTS` (`ssh-keyscan -t ed25519 <ip>`). The job uses the `production` environment, so approvals or branch rules can be added there.
